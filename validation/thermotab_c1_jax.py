...
N_NEWT_INV = 8        # fixed Newton trips for the quintic inverse
...
def make_quintic(Tg, f, d, s):
    ...
    def _locate(T):
        i = jnp.clip(jnp.searchsorted(Tg, T, side="right") - 1, 0, n - 2)
        t = (T - Tg[i]) / D
        return i, t
...
def build_c1(tab):
    ...
    h_f, h_fp = make_quintic(Tg, hg, cpg, cpp)   # h: (h, cp, cp')
    s0_d = cpg / Tg                              # s0' = cp/T at nodes
    s0_s = (cpp * Tg - cpg) / (Tg * Tg)          # (cp/T)' at nodes
    s0_f, s0_fp = make_quintic(Tg, s0g, s0_d, s0_s)
...
def invert_h(c1, ht):
    ...
    fixed N_NEWT_INV trips (converges to roundoff well before)."""
    T0 = jnp.interp(ht, c1["hg"], c1["Tg"])

    def body(_, T):
        return T - (c1["h"](T) - ht) / c1["cp"](T)

    return jax.lax.fori_loop(0, N_NEWT_INV, body, T0)


def state_q_c1(q, c1):
    """C^1 closure q -> (T, p, rho, c, gam, M) — the brick-2 twin of
    [X-A1IM] state_q with the quintic backend."""
    ht = c1["h0"] - 0.5 * q * q
    T = invert_h(c1, ht)
