...
F3  GEOMETRY CLASSES (3-4 sessions; plug/aerospike primary). ENTRY:
    ...
    v2 5/6 FAIL [X-OWSQ] — the pre-registered P3 falsifier fired
    (return in VALUE, 1.17x band_W in LOCATION) and was ATTRIBUTED by
    measurement (AD Hessian at W_fit: the location floor had been
    declared from the stiffest curvature; the softest direction is 32x
    softer) -> instrument v3, bands per eigen-direction, validated on
    Rao's world (6/6, v2 reproduced line by line) and PASS 6/6 on our
    world [X-OWS3] (return 0.19 of band at worst; the start
    discriminated in one direction at 1.5 percent, declared). What this
    discharges: BOTH halves (value + motion) of "certified plug optimum
    + oracle in derived bands on OUR world" for the perfect-expansion
    member (EOS-general gas; gamma-const only in the Rao-world twin).
    STILL NOT for exit: the tournament-grade A/B at the
