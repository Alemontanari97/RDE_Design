...
OD <= 320 mm x L <= 260 mm; CH4/O2 phi = 1, T1 = 300 K, fill 1 atm; feed
equivalence P_cp = 20 atm both engines; CP combustor L* = 1.0 m (LOX/CH4).
Same validated chain as example_design_study/headtohead (asserts on CJ)."""
...
print(f"(c) SK chamber bracket: F/mdot PH={ph['FovM']:.0f} m/s (Term II {100*ph['FII']/ph['FovM']:.0f}%) | axial sonic={ax['FovM_sonic']:.0f} m/s")
...
print(f"(f) RDE @20 atm: bell eps*={ob['eps']:.2f} Isp={ob['Isp']:.1f} s | spike eps*={osp['eps']:.2f} Isp={osp['Isp']:.1f} s (choke margin {s['choke_margin']:.2f})")
